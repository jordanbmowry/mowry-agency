...
export const formatSex = (sex: string | null): string => {
  ...
  return sexMap[sex.toLowerCase()] || sex.charAt(0).toUpperCase() + sex.slice(1).toLowerCase();
};
...
export const formatHealthConditions = (conditions: string | null): string => {
  ...
  if (conditions.toLowerCase() === 'none' || conditions.toLowerCase() === 'n/a') {
...
export const formatMedications = (medications: string | null): string => {
  ...
  if (medications.toLowerCase() === 'none' || medications.toLowerCase() === 'n/a') {
    return 'None reported';
  }
  return medications;
};
 ...
 * Uses the date-fns implementation from dateUtils for consistency
 */
const formatDateForDisplay = (date: string | null | Date): string => {
  if (!date) return 'N/A';
